...
export function ProjectCard({ project, variant = 'grid' }: ProjectCardProps) {
  ...
  return (
    <Link href={`/works/${project.slug}`} className="block group">
      <Card className={`overflow-hidden h-full transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:border-primary/40 ${isFeatured ? 'md:flex md:flex-row' : ''}`}>
        <div className={`relative overflow-hidden bg-muted ${isFeatured ? 'md:w-1/2' : 'aspect-[16/10]'}`}>
          <Image
        ...
        <div className={`p-6 ${isFeatured ? 'md:w-1/2 md:flex md:flex-col md:justify-center' : ''}`}>
