...
export default function RecentWorkCards() {
  ...
  useGSAP(() => {
    ...
    works.forEach((work, index) => {
      //@ts-expect-error
      const cardElement = containerRef.current?.querySelector(`.work-item-${index}`)
      if (!cardElement) return

        const isEven = index % 2 === 0;

        // Calculate dynamic trigger points based on card position and viewport
        const triggerStart = `top ${window.innerHeight * 0.8}`; // Start when 80% of viewport is left
        const triggerEnd = `top ${-window.innerHeight * 0.6}`; // End when card is 60% out of viewport
        ...
        cardTl.fromTo(`.work-item-image-${index}`, 
          {
            x: isEven ? 400 : -400,
            rotate: isEven ? 30 : -30,
            opacity: 0,
          ...
          },
          {
            ...
            scale: 1,
            duration: 2,
            ease: "back.out(1.2)", // Bouncy easing for more natural feel
          },
          0 // Start immediately
        )
        ...
        cardTl.fromTo(`.work-item-text-${index}`,
          {
            y: 120,
            opacity: 0,
            x: isEven ? -30 : 30,
        ...
        ScrollTrigger.create({
          ...
          onEnter: () => {
            gsap.to(cardElement, { scale: 1.02, duration: 0.5, ease: "power2.out" })
          },
          onLeave: () => {
  ...
  return (
    <div ref={containerRef} className='w-full md:max-w-7xl mx-auto my-20 px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 h-full space-y-24 font-custom'>
        {works.map((work, index) => {
            ...
            return (
                ...
                >
                    ...
                    {/* Text Content */}
                    <div className={`work-item-text-${index} w-full md:w-2/5 flex-shrink-0 z-10 ${
                        isEven 
                            ? 'order-2 md:order-1' 
                            : 'order-2 md:order-2 md:text-right'
                    }`}>
                        <h3 className='text-3xl sm:text-4xl md:text-6xl lg:text-7xl font-custom  leading-tight'>
                            {work.title}
                        </h3>
                    ...
                    {/* Image */}
                    <div className={`work-item-image-${index} w-full md:w-3/5 h-[35vh] sm:h-[45vh] md:h-[65vh] lg:h-[75vh] bg-gradient-to-br from-gray-200 to-gray-300 rounded-3xl flex-shrink-0  overflow-hidden   ${
                        isEven 
                            ? 'order-1 md:order-2' 
                            : 'order-1 md:order-1'
                    }`}>
                        <div className='w-full h-full flex items-center justify-center  font-medium text-xl '>
                           <Image src={work.thumbnail} alt={work.title} height="4000" width="4000" className='w-full! h-full! object-cover' />
                        </div>
                    </div>
                </div>
            );
